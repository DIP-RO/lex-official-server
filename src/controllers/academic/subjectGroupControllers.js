...
const getAllSubjectGroup = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await SubjectGroupModel.findById(req.params.id);
    ...
    const data = await SubjectGroupModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await SubjectGroupModel.countDocuments(
      query ? { school: query } : {}
    );
