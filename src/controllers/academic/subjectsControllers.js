...
const getAllSubject = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await SubjectModel.findById(req.params.id);
    ...
    const data = await SubjectModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);
...
export { createSubject, deleteSubject, UpdateSubject, getAllSubject };
