...
const getAllStaffIDCard = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await StaffIDCardModel.findById(req.params.id);
    ...
    const data = await StaffIDCardModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await StaffIDCardModel.countDocuments(
      query ? { school: query } : {}
    );
