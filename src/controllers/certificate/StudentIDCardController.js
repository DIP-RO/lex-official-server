...
const getAllStudentIDCard = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await StudentIDCardModel.findById(req.params.id);
    ...
    const data = await StudentIDCardModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await StudentIDCardModel.countDocuments(
      query ? { school: query } : {}
    );
