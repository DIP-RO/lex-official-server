...
const getAllGenerateCertificates = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await GenerateCertificatesModel.findById(req.params.id);
    ...
    const data = await GenerateCertificatesModel.find(
      query ? { school: query } : {}
    )
      .sort({ created_at: -1 })
      .skip(startIndex)
    ...
    const count = await GenerateCertificatesModel.countDocuments(
      query ? { school: query } : {}
    );
