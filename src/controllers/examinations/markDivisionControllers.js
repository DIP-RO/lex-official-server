...
const getAllMarkDivision = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await MarkDivisionModel.findById(req.params.id);
    ...
    const data = await MarkDivisionModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await MarkDivisionModel.countDocuments(
      query ? { school: query } : {}
    );
