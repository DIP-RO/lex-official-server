...
const getAllMarkGrade = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await MarkGradeModel.findById(req.params.id);
    ...
    const data = await MarkGradeModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await MarkGradeModel.countDocuments(
      query ? { school: query } : {}
    );
...
export { createMarkGrade, deleteMarkGrade, UpdateMarkGrade, getAllMarkGrade };
