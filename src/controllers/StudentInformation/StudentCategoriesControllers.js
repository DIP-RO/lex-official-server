...
const getAllStudentCategories = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await StudentCategoriesModel.findById(req.params.id);
    ...
    const data = await StudentCategoriesModel.find(
      query ? { school: query } : {}
    )
      .sort({ created_at: -1 })
      .skip(startIndex)
    ...
    const count = await StudentCategoriesModel.countDocuments(
      query ? { school: query } : {}
    );
