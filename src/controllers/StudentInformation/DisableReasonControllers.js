...
const getAllDisableReasons = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await DisableReasonModel.findById(req.params.id);
    ...
    const data = await DisableReasonModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await DisableReasonModel.countDocuments(
      query ? { school: query } : {}
    );
