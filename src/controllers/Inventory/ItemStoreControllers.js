...
const getAllItemStore = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await ItemStoreModel.findById(req.params.id);
    ...
    const data = await ItemStoreModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await ItemStoreModel.countDocuments(
      query ? { school: query } : {}
    );
