...
const getAllItemCategory = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await ItemCategoryModel.findById(req.params.id);
    ...
    const data = await ItemCategoryModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await ItemCategoryModel.countDocuments(
      query ? { school: query } : {}
    );
