...
const getAllAddItemStock = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await AddItemStockModel.findById(req.params.id);
    ...
    const data = await AddItemStockModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await AddItemStockModel.countDocuments(
      query ? { school: query } : {}
    );
