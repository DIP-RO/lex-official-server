...
const getAllAddItem = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await AddItemModel.findById(req.params.id);
    ...
    const data = await AddItemModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await AddItemModel.countDocuments(
      query ? { school: query } : {}
    );
