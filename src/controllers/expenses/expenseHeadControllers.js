...
const getAllExpenseHead = async (req, res) => {
  try {
    if (req.params.id) {
      const data = await ExpenseHeadModel.findById(req.params.id);
    ...
    const data = await ExpenseHeadModel.find(query ? { school: query } : {})
      .sort({ created_at: -1 })
      .skip(startIndex)
      .limit(limit);

    const count = await ExpenseHeadModel.countDocuments(
      query ? { school: query } : {}
    );
